...
class MassSMSTest(models.TransientModel):
    ...
    def action_send_sms(self):
        ...
        if allow_custom_sms:
            self.action_custom_provider_send_sms()
        else:
            super(MassSMSTest, self).action_send_sms()

        return True

    def action_custom_provider_send_sms(self):
        self.ensure_one()
        ...
        sent_sms_list = self.env['sms_cm.config']._send_sms_batch([{
            'res_id': number,
            'phone': number,
            'message': body,
